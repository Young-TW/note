...
雖然目前 KV Cache 有命中，但測試時實際加速不明顯，可能是因為測試的 prompt 較短，後續會嘗試更長的 prompt 來測量效能。

## 接下來的規劃

1. 加長測試的 prompt 長度，觀察 LMCache 命中率與效能提升
2. 測試更多不同語言模型與 vLLM 版本的相容
3. 嘗試透過腳本自動化部署 vLLM + LMCache 環境
4. 測試不同 GPU 數量 Decode/Prefill 的組合是否正常執行
5. 測量不同組合的效能表現
